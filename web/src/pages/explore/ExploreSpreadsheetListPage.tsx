...
const ExploreSpreadsheetListPage = () => {
    const { t } = useTranslation()
    const navigate = useNavigate()

    const { data: views, isLoading } = useQuery({
        queryKey: ['publicviews', 'spreadsheet'],
        queryFn: async () => {
            const allViews = await getPublicViews(1, 100, 'spreadsheet')
            return allViews
        },
    })
                ...
                <div className="py-4">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3 h-10">
                            <SidebarButton />
                            <PublicViewMenu viewType="spreadsheet" />
                        </div>
                    </div>
...
export default ExploreSpreadsheetListPage
