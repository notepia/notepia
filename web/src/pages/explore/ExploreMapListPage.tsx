...
const ExploreMapListPage = () => {
    const { t } = useTranslation()
    const navigate = useNavigate()

    const { data: views, isLoading } = useQuery({
        queryKey: ['publicviews', 'map'],
        queryFn: async () => {
            const allViews = await getPublicViews(1, 100, 'map')
            return allViews
        },
    })
...
export default ExploreMapListPage
