...
const CalendarListPage = () => {
    ...
    const { data: views, isLoading } = useQuery({
        queryKey: ['views', currentWorkspaceId, 'calendar'],
        queryFn: async () => {
            const allViews = await getViews(currentWorkspaceId)
            return allViews.filter((v: any) => v.type === 'calendar')
        },
    ...
    useEffect(() => {
        if (views && views.length > 0) {
            navigate(`/workspaces/${currentWorkspaceId}/calendar/${views[0].id}`, { replace: true })
        }
